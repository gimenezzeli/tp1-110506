...
informacion_pokemon_t *ordenar_alfabeticamente(informacion_pokemon_t *ip){
	...
	for (int i = 0; i < ip->cant_pokemones; i++){
		...
		for (int j = i+1; j < ip->cant_pokemones; j++){
			int comparacion = strcmp(ip->pokemones[j].nombre,ip->pokemones[min].nombre);
			if (comparacion<0){
				min=j;
			}
		}
...
**/
struct info_pokemon *cargar_poke_valido(struct pokemon *pokemon_aux, struct info_pokemon *info){
	info->pokemones = realloc(info->pokemones, sizeof(struct pokemon)*(unsigned long)(1+info->cant_pokemones));
	if (info->pokemones==NULL){
		free(pokemon_aux);
		free(info->pokemones);
	}
	strcpy(info->pokemones[info->cant_pokemones].nombre, pokemon_aux->nombre);
...
informacion_pokemon_t *pokemon_cargar_archivo(const char *path) {
	...
	while (fgets(linea, MAX_LINEA, archivo) != NULL && seguir_leyendo==true){
		...
		if (contador_delimitadores==1){
			...
			int ataques_valores_validos = sscanf(linea,"%[^;];%c;%u\n", nombre_ataque, &tipo_ataque, &poder);
			if ( ataques_valores_validos == 3 && tipo_valido(tipo)){
				strcpy(pokemon_aux.ataques[pokemon_aux.cant_ataques].nombre, nombre_ataque);
				pokemon_aux.ataques[pokemon_aux.cant_ataques].tipo = parsear_tipo(tipo_ataque);
				pokemon_aux.ataques[pokemon_aux.cant_ataques].poder = poder;
				pokemon_aux.cant_ataques++;
				if (pokemon_aux.cant_ataques==3){
					info = cargar_poke_valido(&pokemon_aux, info);
					pokemon_aux.cant_ataques = 0;
				}
				
			} else if ( ( ataques_valores_validos != 3 || !tipo_valido(tipo)) && info->cant_pokemones==0){
				seguir_leyendo = false;
				//free(pokemon_aux);
				free(info->pokemones);
				free(info);
